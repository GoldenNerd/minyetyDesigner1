...
// Assert ("hardcode") script serial number
  ...
  document.querySelector("#password").innerText =`Serial Number "${hardwareSerialNum}" matched successfully.`;
...
*/
 ...
 Smaller is 1. Larger is 6. */
  ...
  curvedSlatVerticalContribution: 2.9,
  flatSlatVerticalContribution: 2.6,
...
const calcDat = {
  softwareSerialNumber: "placeholderForS/N",
...
// (@2)
function calcRoDrForHgoal(hGoal=calcDat.approximateHGoal) {
  let nthIteration;
 ...
 // §
      if (nthTableRow.h >= hGoal) {
        ...
        console.log(
          `💪 Success! Found rO ${
            Math.round(10000 * nthTableRow.rO) / 10000
          }  For an HGoal ${
            Math.round(10000 * hGoal) / 10000
          }`
        );
...
function openHangingSlatCount() {
  ...
  const slatCount =
    calcDat.openHangingHeight / calcDat.slatVerticalContribution;
...
function closedHangingSlatCount() {
  // Substract 1 that belongs to the BB
  const slatCount =
    calcDat.closedHangingHeight / calcDat.slatVerticalContribution;
...
function unspoolSpringSpecs () {
  ...
  document
    .querySelector(":root")
    .style.setProperty("--blink-color", "rgb(25,25,190)"); // #151530 #291609
}
...
// @1 approximate hGoal is inside buildCalcDat()()
  buildCalcDat();
 //(@2) approximate rO
  calcRoDrForHgoal();
